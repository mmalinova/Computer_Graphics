// Daisy_Win32_Project_MM.cpp : Defines the entry point for the application.
//

#include "stdafx.h"
#include "Daisy_Win32_Project_MM.h"
#include "GdiPlus.h"
	...
	_In_ int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_DAISY_WIN32_PROJECT_MM, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_DAISY_WIN32_PROJECT_MM));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_DAISY_WIN32_PROJECT_MM));
	wcex.hCursor = LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCE(IDC_DAISY_WIN32_PROJECT_MM);
...
VOID DRAW_Daisy(HDC hdc, int ak, int xend, int yend)
{
	...
	case 1:
	{
			  graphics.DrawBezier(&greenPen, xend / 2.07, yend / 2.85, xend / 2, yend / 2.4, xend / 2.3, yend / 2, xend / 2.1, yend / 1.3);
	}
		break;
	case 2:
	{
			  graphics.DrawEllipse(&blackPen, circleRect); 
			  graphics.FillEllipse(&yellowBrush, circleRect);
	}
		break;
	case 3:
	{
	...
	{
			   myPath1.AddBezier(p[0], p[1], p[2], p[3]);
			   myMatrix.RotateAt(-225, center);
			   myPath1.Transform(&myMatrix);
	}
		break;
	}
	
	graphics.DrawPath(&blackPen, &myPath1);
	graphics.FillPath(&whiteBrush, &myPath1);
	myPath1.CloseFigure();

	graphics.FillPath(&Gardenbr, &myPath);
}
