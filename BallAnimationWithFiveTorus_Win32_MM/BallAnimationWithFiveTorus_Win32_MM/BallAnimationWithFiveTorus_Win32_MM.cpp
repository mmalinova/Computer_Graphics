// BallAnimationWithFiveTorus_Win32_MM.cpp : Defines the entry point for the application.
//

#include "stdafx.h"
#include "BallAnimationWithFiveTorus_Win32_MM.h"
#include "gl/gl.h"
#include "gl/glu.h"
#include "glut.h"
                     ...
                     _In_ int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_BALLANIMATIONWITHFIVETORUS_WIN32_MM, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_BALLANIMATIONWITHFIVETORUS_WIN32_MM));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_BALLANIMATIONWITHFIVETORUS_WIN32_MM));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_BALLANIMATIONWITHFIVETORUS_WIN32_MM);
...
void Render()
{
	...
	// определяне на яркост на светене
	glLightf(GL_LIGHT1, GL_SPOT_CUTOFF, 70.0);
	glShadeModel(GL_SMOOTH);
	glEnable(GL_SMOOTH);

	// определяне на ъглова стъпка на завъртане за фигурите
	Ball_Angle = 90.0;
	TorusAngle += 2.35;

	//определяне на позицията на наблюдателя
	if (camera == 1) {
		gluLookAt(0.0, ball, 20.0, 0.0, -5.0, -10.0, 0.0, 1.0, 0.0);
	}
	else if (camera == 2) {
		gluLookAt(0.0, -ball, 20.0, 0.0, -5.0, -10.0, 0.0, 1.0, 0.0);
	}
	else if (camera == 3) {
		gluLookAt(0.0, 25.0, -5.0, 0.0, 0.0, -5.0, 0.0, 0.0, 1.0);
	}
	else if (camera == 4) {
		gluLookAt(0.0, -25.0, -5.0, 0.0, 0.0, -5.0, 0.0, 0.0, 1.0);
	}
	else if (camera == 5) {
		gluLookAt(0.0, 20.0, 10.0, 0.0, 0.0, -5.0, 0.0, 1.0, 0.0);
	}
	if (camera > 5)
		camera = 1;
	...
	//отместване на топчето
	ball -= 0.14;
	if (ball < -16.0) {
		ball = 5;
		camera++;
	}
	glTranslatef(0.0, ball, 0.0);

	// задаване на цвят на топчето  
	glColor3f(0.8, 0.0, 0.2);
	glutSolidSphere(1.0, 50, 20); // плътен модел на топчето
	glPopMatrix();

	//позициониране на топчето
	glPushMatrix();

	// въртене на топчето
	glRotatef(Ball_Angle, 1.0, 0.0, 0.0);

	// задаване на цветове на торусите
	glPushMatrix();
	if (ball > 6)
	...
	glRotatef(TorusAngle, 1.0, 1.0, 1.0);
	glutSolidTorus(0.8, 2.5, 30, 40); // плътен модел на торус
	glPopMatrix();
	...
	glRotatef(TorusAngle, 1.0, 1.0, 0.0);
	glutSolidTorus(0.8, 2.5, 30, 40); // плътен модел на торус
	glPopMatrix();
