// BallAnimation_Win32ProjectMM.cpp : Defines the entry point for the application.
//

#include "stdafx.h"
#include "BallAnimation_Win32ProjectMM.h"
#include "gl/gl.h"
#include "gl/glu.h"
#include "glut.h"
                     ...
                     _In_ int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_BALLANIMATION_WIN32PROJECTMM, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_BALLANIMATION_WIN32PROJECTMM));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_BALLANIMATION_WIN32PROJECTMM));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_BALLANIMATION_WIN32PROJECTMM);
...
{ 
	...
	if (camera == 1) {
		...
		gluLookAt(0.0, 20.0, 15.0, 0.0, 0.0, -5.0, 0.0, 1.0, 0.0);
	}
	if (camera > 5)
		camera = 1;
	...
	glutSolidSphere(2, 50, 20); // плътен модел на топчето
	glPopMatrix();
	...
	// задаване на цвят на торуса
	if (ball < -1.0)
		glColor3f(1.0, 0.3, 0.0);
	else
		glColor3f(0.7, 1.0, 0.0);
	glutSolidTorus(1.5, 4.0, 30, 40); // плътен модел на торус
	glPopMatrix();
